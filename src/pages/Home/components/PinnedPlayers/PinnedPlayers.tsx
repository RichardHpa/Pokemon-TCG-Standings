...
interface PinnedPlayersContextProps {
  ...
  togglePinPlayer: (tournamentId: string, playerName: string, division: Division) => void;
  // isPinned: (tournamentId: string, playerName: string, division: Division) => boolean;
}
...
export const PinnedPlayersProvider = ({ children }: { children: ReactNode }) => {
  ...
  //   localStorage.getItem(pinnedPlayersKey) JSON.parse(pinnedPlayers)|| JSON.stringify({})
  // );
  const [pinnedPlayers, setPinnedPlayers] = useState(() => {
    const storedPinnedPlayers = localStorage.getItem(pinnedPlayersKey);
    const parsedPinnedPlayers = storedPinnedPlayers ? JSON.parse(storedPinnedPlayers) : {};
    return parsedPinnedPlayers;
  });

  // console.log('pinnedPlayersState', pinnedPlayers);
  ...
  const unPinPlayer = useCallback(
    (tournamentId: string, playerName: string, division: Division) => {
      ...
      // }
      // const newPlayers = JSON.stringify(parsedPinPlayers);
      // setPinnedPlayers(newPlayers);
      // localStorage.setItem(pinnedPlayersKey, newPlayers);
    },
    []
  );
  ...
  const togglePinPlayer = useCallback(
    (tournamentId: string, playerName: string, division: Division) => {
      // console.log(tournamentId, playerName, division);
      // console.log(pinnedPlayers);
      const parsedPinPlayers = pinnedPlayers;
      if (!parsedPinPlayers[tournamentId]) {
        parsedPinPlayers[tournamentId] = {
          [division]: [playerName],
        };
      } else if (!parsedPinPlayers[tournamentId][division]) {
        parsedPinPlayers[tournamentId][division] = [playerName];
      } else if (parsedPinPlayers[tournamentId][division].includes(playerName)) {
        parsedPinPlayers[tournamentId][division] = parsedPinPlayers[tournamentId][division].filter(
          (p: string) => p !== playerName
        );
      } else {
        parsedPinPlayers[tournamentId][division].push(playerName);
      }
      if (parsedPinPlayers[tournamentId][division].length === 0) {
        delete parsedPinPlayers[tournamentId][division];
      }
      ...
      console.log(parsedPinPlayers);
      // const newPlayers = JSON.stringify(parsedPinPlayers);
      setPinnedPlayers(parsedPinPlayers);
      // localStorage.setItem(pinnedPlayersKey, newPlayers);
    },
    [pinnedPlayers, setPinnedPlayers]
  );

  // const isPinned = (tournamentId: string, playerName: string, division: Division) => {
  //   return pinnedPlayers[tournamentId]?.[division]?.includes(playerName);
  // };
  ...
  //   [pinnedPlayers]
  // );
  // const isPinned = useMemo((tournamentId: string, playerName: string, division: Division) => {
  //   return pinnedPlayers[tournamentId]?.[division]?.includes(playerName);
  // }, []);

  const value = useMemo(() => {
    return { pinnedPlayers, unPinPlayer, togglePinPlayer };
  }, [pinnedPlayers, unPinPlayer]);

  return <PinnedPlayersContext.Provider value={value}>{children}</PinnedPlayersContext.Provider>;
};

export const usePinnedPlayers = () => useContext(PinnedPlayersContext);
