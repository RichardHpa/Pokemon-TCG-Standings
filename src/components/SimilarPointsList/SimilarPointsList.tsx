...
export const SimilarPointsList: FC<SimilarPointsProps> = ({
    player,
    data,
    totalPoints,
}) => {
        ...
        if (length > 4) {
            return (
                <>
                    <ResistancesListItem
                        state="above"
                        player={players.above[0]}
                    />
                    <ResistancesListItem
                        state="above"
                        player={players.above[1]}
                    />
                    ...
                    <ResistancesListItem
                        state="above"
                        player={players.above[length - 2]}
                    />
                    <ResistancesListItem
                        state="above"
                        player={players.above[length - 1]}
                    />
                </>
            );
        }
        ...
        if (length > 4) {
            return (
                <>
                    <ResistancesListItem
                        state="below"
                        player={players.below[0]}
                    />
                    <ResistancesListItem
                        state="below"
                        player={players.below[1]}
                    />
                    ...
                    <ResistancesListItem
                        state="below"
                        player={players.below[length - 2]}
                    />
                    <ResistancesListItem
                        state="below"
                        player={players.below[length - 1]}
                    />
                </>
            );
        }
        ...
        return (
            <div className="py-2.5 px-3">
                <p className="my-3 font-semibold leading-tight text-gray-800 dark:text-gray-400 text-sm">
                    Player comparisons will be available after round 3 as there
                    would be to many players to compare
                </p>
            </div>
        );
    }
    ...
    return (
        <>
            <div className="py-2.5 px-3">
                <p className="my-3 font-semibold leading-tight text-gray-800 dark:text-gray-400 text-sm">
                    There are {playerCount} players with also {totalPoints}{' '}
                    Points
                </p>
            </div>
