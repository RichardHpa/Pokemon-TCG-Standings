        ...
        <div className="text-sm font-medium divide-y divide-gray-200 dark:divide-gray-700">
          {players.map(player => {
            return (
              ...
              >
                <span>
                  ({getFirstLetterAndUpperCase(player.division)}) {player.player.placing} -{' '}
                  {player.player.name}
                </span>
                <PlayerRecord record={player.player.record} colors={true} />
              </Link>
            );
          })}
        </div>
          ...
          return (
            ...
            >
              {({ data, index, style }: ListChildComponentProps) => (
                <div style={style}>
                  <Row data={data} index={index} setSize={setSize} closeDrawer={closeDrawer} />
                </div>
              )}
            </List>
          );
